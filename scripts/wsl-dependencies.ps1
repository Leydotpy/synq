...
function Start-MeetWslDependencies {
    ...
    $redisPort = Get-EnvironmentValueOrDefault -Names @("MEET_WSL_REDIS_PORT", "WHITEBOARD_WSL_REDIS_PORT") -Default "6379"
    $janusWsPort = Get-EnvironmentValueOrDefault -Names @("MEET_WSL_JANUS_WS_PORT", "WHITEBOARD_WSL_JANUS_WS_PORT") -Default "8188"
    $redisCommand = Get-EnvironmentValueOrDefault -Names @("MEET_WSL_REDIS_COMMAND", "WHITEBOARD_WSL_REDIS_COMMAND") -Default "redis-server --bind 127.0.0.1 --port $redisPort --daemonize no"
...
start_dependency() {
    ...
    local start_result method service_name pid log_tail
    ...
    if [ -n "$start_result" ]; then
        ...
        if wait_until_ready "$ready_fn"; then
            emit "$name" "started" "$method" "$service_name" "$pid"
            return 0
        fi
...
function Stop-MeetWslDependencies {
    ...
    $managedDependencies = @(
        $state.dependencies |
            ...
            ForEach-Object {
                "$($_.name)`t$($_.method)`t$($_.serviceName)`t$($_.processId)"
            }
    )
...
while IFS="$(printf '\t')" read -r name method service_name pid; do
    [ -z "$name" ] && continue
    case "$method" in
        direct)
            stop_direct "$name" "$pid" || true
            ;;
        systemd|service)
